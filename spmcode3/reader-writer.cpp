//
// Multiple Reades Single Writer pattern
// This simple version does not guarantee strict fairness
...
int main() {
	...
	auto reader = [&](int count, int id) {
		for(int i=0;i<count; ++i) {
			std::shared_lock<std::shared_mutex> lock(smutex);   // shared_lock for non-exclusive access
	...
	auto writer = [&](int count, int id) {
		for(int i=0;i<count; ++i) {
			std::unique_lock<std::shared_mutex> lock(smutex);  // unique lock to gain exclusive access
			++shared_counter;
