...
// variant that improves the block distribution
// using a more balanced static partitioning
template <
    typename value_t,
    typename index_t>
void block_parallel_mult2(
    std::vector<value_t>& A,
    std::vector<value_t>& x,
    std::vector<value_t>& b,
    index_t m,
    ...
    auto block = [&] (const index_t& id) -> void {
        // compute chunk size, lower and upper task id
        const index_t chunk = m / num_threads;
		const index_t rem   = m % num_threads;
        const index_t lower = id*chunk + std::min(id, rem);
        const index_t upper = lower + chunk + (id<rem?1:0);
...
void block_cyclic_parallel_mult(
    ...
    auto block_cyclic = [&] (const index_t& id) -> void {
        ...
        for (index_t lower = offset; lower < m; lower += stride) {	
            ...
            for (index_t row = lower; row < upper; row++) {
				const index_t row_offset = row * n;
				value_t accum = value_t(0);
				for (index_t col = 0; col < n; col++)
                    accum += A[row_offset+col]*x[col];
                b[row] = accum;
            }
		}
    };
...
int main(int argc, char* argv[]) {
	...
	TIMERSTART(cyclic_falsesharing);
	cyclic_parallel_mult_falsesharing(A, x, b, m, n, nthreads);
	TIMERSTOP(cyclic_falsesharing);

	check(b,m, n);
	
	TIMERSTART(cyclic);
	cyclic_parallel_mult(A, x, b, m, n, nthreads);
	TIMERSTOP(cyclic);

	check(b,m, n);
	
    TIMERSTART(block);
	block_parallel_mult2(A, x, b, m, n, nthreads);
    TIMERSTOP(block);

	check(b,m, n);

	TIMERSTART(block_cyclic);
	block_cyclic_parallel_mult(A, x, b, m, n, nthreads, chunk_size);
