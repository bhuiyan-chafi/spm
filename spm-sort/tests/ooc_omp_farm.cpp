...
namespace farm
{
                      ...
                      int writer_id)
    {
        ...
        while (true)
        {
            ...
            {
                spdlog::info("[Writer-{} TID:{}] Poison received, wrote {} segments",
            ...
            // WriteTask goes out of scope here - memory freed
            spdlog::info("[Writer-{} TID:{}] Completed segment {} in {}ms - Memory freed + trimmed",
                         writer_id, tid, write_task.segment_id, duration_ms); // Add path to shared list (synchronized)
            {
        ...
        spdlog::info("[Writer-{} TID:{}] Finished: {} segments written",
                     writer_id, tid, segments_written);
    }
    ...
    void run_farm(size_t num_workers, size_t num_writers = 2)
    {
        ...
        SafeQueue<Task> task_queue(num_workers);
        SafeQueue<SortedTask> sorted_queue(num_workers);
...
#pragma omp task firstprivate(i)
                    {
                        writer_stage(write_queue, run_paths, paths_mutex, i);
                    }
                }
            }
        }
        ...
        uint64_t total_worker_ms = total_worker_time_ms.load();
        size_t total_tasks = tasks_sorted.load();
        double total_worker_seconds = total_worker_ms / 1000.0;
        double total_worker_minutes = total_worker_seconds / 60.0;

        spdlog::info("==> Total tasks sorted: {} <==", total_tasks);
        spdlog::info("==> Total worker time (accumulated): {} ms ({:.2f} s / {:.2f} min) <==",
                     total_worker_ms, total_worker_seconds, total_worker_minutes);

        // Calculate average time per task
        if (total_tasks > 0)
        {
            double avg_time_per_task_ms = static_cast<double>(total_worker_ms) / total_tasks;
            spdlog::info("==> Average time per task: {:.2f} ms <==", avg_time_per_task_ms);
        }
...
int main(int argc, char **argv)
{
    ...
    // Parse total_time result to get milliseconds
    std::string time_result = total_time.result();
    double wall_clock_seconds = 0.0;

    // Extract seconds from format like "1234.56s" or "12.34ms"
    if (time_result.find("ms") != std::string::npos)
    {
        wall_clock_seconds = std::stod(time_result.substr(0, time_result.find("ms"))) / 1000.0;
    }
    else if (time_result.find("s") != std::string::npos)
    {
        wall_clock_seconds = std::stod(time_result.substr(0, time_result.find("s")));
    }

    spdlog::info("");
    spdlog::info("╔════════════════════════════════════════════════════════════════╗");
    spdlog::info("║              FINAL EXECUTION SUMMARY                           ║");
    spdlog::info("╚════════════════════════════════════════════════════════════════╝");
    spdlog::info("");
    spdlog::info("  Total Execution Time (Wall Clock): {}", total_time.result());
    spdlog::info("  Output File: {}", DATA_OUTPUT);
    spdlog::info("");

    if (wall_clock_seconds > 0)
    {
        // Calculate throughput
        double throughput_mb_per_sec = (INPUT_BYTES / (1024.0 * 1024.0)) / wall_clock_seconds;
        double throughput_gb_per_min = (INPUT_BYTES / (1024.0 * 1024.0 * 1024.0)) * (60.0 / wall_clock_seconds);

        spdlog::info("  Processing Throughput:");
        spdlog::info("    - {:.2f} MB/s", throughput_mb_per_sec);
        spdlog::info("    - {:.2f} GB/min", throughput_gb_per_min);
        spdlog::info("");
    }

    spdlog::info("╔════════════════════════════════════════════════════════════════╗");
    spdlog::info("║                 ✓ COMPLETED SUCCESSFULLY ✓                     ║");
    spdlog::info("╚════════════════════════════════════════════════════════════════╝");
    spdlog::info("");
