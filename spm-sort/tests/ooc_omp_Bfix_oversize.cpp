/**
 * Memory is not bloating for oversized files but is bloating for small size. The issue is std::sort() holds O(N) space at worst case
 */
#include "main.hpp"
...
namespace omp_sort
{
    namespace
    {
        ...
        std::vector<std::string> process_segment(std::unique_ptr<Items> segment, size_t segment_id,
                                                 size_t threads, bool is_spill)
        {
...
#pragma omp critical
            ...
            // For in-memory case, we need to handle tasks differently
            if (!is_spill)
            {
                // Merge sorted slices from this segment and write to a temp file
                const auto id = run_id.fetch_add(1, std::memory_order_relaxed);
                const std::string path = DATA_TMP_DIR + "run_" + std::to_string(id) + ".bin";

                std::ofstream out(path, std::ios::binary);
                if (!out)
                    throw std::runtime_error("Cannot open temp file for in-memory segment merge");
        ...
        void run_pipeline(size_t threads)
        {
            ...
            {
                TimerScope ts_emit(emitter_time);
                TimerScope ts_work(worker_time);
                ...
                while (true)
                {
                    ...
                    spdlog::info("Processing segment {} ({} items, spill={})",
                                 segment_id, segment->size(), reader.any_spill ? "true" : "false");

                    auto run_paths = process_segment(std::move(segment), segment_id,
                                                     threads ? threads : 1, reader.any_spill);

                    all_run_paths.insert(all_run_paths.end(), run_paths.begin(), run_paths.end());
                    ++segment_id;

                    // Memory freed here before loading next segment
                }
            }
