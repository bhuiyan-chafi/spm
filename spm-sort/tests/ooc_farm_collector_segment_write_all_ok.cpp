...
namespace farm
{
    ...
    void collector_stage(SafeQueue<SortedTask> &sorted_queue, size_t num_workers)
    {
        ...
        // Handle any remaining incomplete segments (edge case)
        if (!segments.empty())
        {
        ...
        // Final merge of all segment runs
        if (run_paths.empty())
        {
            // No runs created (shouldn't happen but handle gracefully)
            std::ofstream(DATA_OUTPUT, std::ios::binary);
        }
        else if (run_paths.size() == 1)
        {
            // Only one run - just rename it
            spdlog::info("[Collector] Single run file - renaming to output");
            std::filesystem::rename(run_paths[0], DATA_OUTPUT);
        }
        else
        {
            // Multiple runs - perform final k-way merge
