/**
 * @file openmpv2.cpp
 * @brief the second version I built with OPENMP. Has fixed some issues from openmpv1, but it still has several issues, but let me point some major ones:
 * - achieved some level of distribution but still not efficient
 * - idle workers
 * - memory bloat partially fixed
 * - no parallelism to be honest except OPENMP pragmas
 * - static scheduling of tasks
 * @file ./discussions/openmpv2.md
...
namespace omp_sort
{
    namespace
    {
        ...
        struct Task
        {
            ...
            size_t segment_id;
            size_t slice_index;
            std::string spill_path; // Path to the temp file if spilled
        };
...
#pragma omp parallel for schedule(dynamic) num_threads(static_cast<int>(threads))
            ...
            for (size_t i = 0; i < tasks.size(); ++i)
            {
                ...
                if (task.spill)
                {
                    ...
                    task.spill_path = path; // Store the path for later collection
        ...
        std::vector<std::string> collect_to_temp_files(std::vector<Task> &tasks)
        {
            ...
            if (any_spill)
            {
                ...
                for (size_t i = 0; i < tasks.size(); ++i)
                {
                    if (tasks[i].spill && !tasks[i].spill_path.empty())
                    {
                        run_paths.push_back(tasks[i].spill_path);
                    }
                }
            }
            else
        ...
        {
            /**
             * each time we create a scope and destroy
             * the time will be incremented instead of replaced
             * that's why until the whole program ends, it keeps accumulating
             */
            ...
            while (true)
            {
                std::unique_ptr<Items> segment;
                {
                    ...
                    // here we are deciding if we are going to the read the segment or it was a single segment
                    if (!segment)
                        break;
                }
                // true for IN_MEMORY operation and for last segment of OOC operation
                ...
                // which proves that we are reading -> processing a single segment at a time
                std::vector<Task> tasks;
                {
